...
SELECT 
    fertigungsstation.bezeichnung AS station_name,
    COUNT(alarm.ID) AS anzahl_alarme
FROM 
    alarm
JOIN 
    fertigungsstation ON alarm.station_id = fertigungsstation.ID
JOIN 
    fertigungslinie ON fertigungsstation.fertigungslinie_id = fertigungslinie.ID
GROUP BY 
    fertigungsstation.bezeichnung
ORDER BY 
    anzahl_alarme DESC;
...
SELECT 
    ...
    (SELECT SUM(anzahl) 
     FROM auftrag_batches 
     WHERE auftrag_batches.fertigungslinie_id = fertigungslinie.ID) AS gesamt_produzierte_menge
FROM 
    fertigungslinie
WHERE 
    (SELECT SUM(anzahl) 
     FROM auftrag_batches 
     WHERE auftrag_batches.fertigungslinie_id = fertigungslinie.ID) > 8000
ORDER BY 
    gesamt_produzierte_menge DESC;
